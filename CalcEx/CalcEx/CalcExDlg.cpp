...
void CCalcExDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_KOR, m_nKor);
...
BEGIN_MESSAGE_MAP(CCalcExDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_CAL, &CCalcExDlg::OnClickedButtonCal)
END_MESSAGE_MAP()
...
void CCalcExDlg::OnClickedButtonCal()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData(TRUE);

	m_nSum = m_nKor + m_nEng + m_nMath;
	m_nAvg = m_nSum / 3;
	
	if (m_nAvg >= 90) m_strGrade = _T("A");
	else if (m_nAvg >= 80) m_strGrade = _T("B");
	else if (m_nAvg >= 70) m_strGrade = _T("C");
	else if (m_nAvg >= 60) m_strGrade = _T("D");
	else m_strGrade = _T("F");
